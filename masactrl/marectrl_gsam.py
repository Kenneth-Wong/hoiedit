...
class MutualRelGSAMControlMaskAuto(MutualSelfAttentionControl):
    def __init__(self, source_mask, target_mask, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50, thres=0.1,
                 mask_save_dir=None, model_type="SD"):
        """
        ...
        print("Using MutualRelGSAMControlMaskAuto")
        self.thres = thres
        self.source_mask = torch.from_numpy(source_mask).float()  # 2, 512, 512
        self.target_mask = torch.from_numpy(target_mask).float()  # 2, 512, 512
        self.save_source_flag = False
        self.save_target_flag = False
    ...
    def attn_so_batch(self, q, k, v, mask, num_heads, **kwargs):
        """
        Performing attention for a batch of queries, keys, and values
        """
        B = q.shape[0] // num_heads
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        source_masks = self.source_mask.to(q.device)
        res = int(np.sqrt(q.shape[1]))
        source_masks = F.interpolate(source_masks.unsqueeze(0), (res, res)).flatten(2).squeeze(0)
        if self.mask_save_dir is not None and not self.save_source_flag:
            H = W = int(np.sqrt(source_masks.shape[1]))
            mask_image = source_masks.reshape(-1, H, W).unsqueeze(1)  # 2, 1, res, res  for B,C,H,W saving
            save_image(mask_image, os.path.join(self.mask_save_dir, f"mask_s.png"), pad_value=0.5)
            # save_image(mask_image[0],
            #            os.path.join(self.mask_save_dir, f"subj_mask_s_{self.cur_step}_{self.cur_att_layer}.png"))
            # save_image(mask_image[1],
            #            os.path.join(self.mask_save_dir, f"obj_mask_s_{self.cur_step}_{self.cur_att_layer}.png"))
            self.save_source_flag = True
        out_u_target = self.attn_so_batch(qu[-num_heads:], ku[:num_heads], vu[:num_heads], source_masks, num_heads, **kwargs)
        out_c_target = self.attn_so_batch(qc[-num_heads:], kc[:num_heads], vc[:num_heads], source_masks, num_heads, **kwargs)

        target_masks = self.target_mask.to(q.device)
        target_masks = F.interpolate(target_masks.unsqueeze(0), (res, res)).flatten(2).squeeze(0).unsqueeze(-1)
        if self.mask_save_dir is not None and not self.save_target_flag:
            H = W = int(np.sqrt(target_masks.shape[1]))
            mask_image = target_masks.reshape(-1, H, W).unsqueeze(1)  # 2, 1, res, res  for B,C,H,W saving
            save_image(mask_image, os.path.join(self.mask_save_dir, f"mask_t.png"), pad_value=0.5)
            self.save_target_flag = True
        ...
        out_u_target = out_u_target_obj_fg * target_masks[1] + out_u_target_subj_fg * target_masks[0] + out_u_target_bg * (target_masks.sum(0) == 0)  #+ #
        out_c_target = out_c_target_obj_fg * target_masks[1] + out_c_target_subj_fg * target_masks[0] + out_c_target_bg * (target_masks.sum(0) == 0)  #+   #
        ...
        #         save_image(mask_image, os.path.join(self.mask_save_dir,
        #                                             f"binarized_mask_s_{self.cur_step}_{self.cur_att_layer}.png"))
        #     out_u_target = self.attn_so_batch(qu[-num_heads:], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        #     out_c_target = self.attn_so_batch(qc[-num_heads:], kc[:num_heads], vc[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        #
        # if self.self_attns_mask is not None:
        ...
        #         mask_image[mask_image >= self.thres] = 1
        #         mask_image[mask_image < self.thres] = 0
        #         save_image(mask_image, os.path.join(self.mask_save_dir, f"binarized_mask_t_{self.cur_step}_{self.cur_att_layer}.png"))
        #     # binarize the mask
        #     thres = self.thres
        #     spatial_mask[spatial_mask >= thres] = 1
        #     spatial_mask[spatial_mask < thres] = 0
        #     # out_u_target_subj_fg, out_u_target_obj_fg, out_u_target_subj_bg, out_u_target_obj_bg = out_u_target.chunk(4)
        #     # out_c_target_subj_fg, out_c_target_obj_fg, out_c_target_subj_bg, out_c_target_obj_bg = out_c_target.chunk(4)
        #     #
        #     # out_u_target = out_u_target_subj_fg * spatial_mask[0] + out_u_target_obj_fg * spatial_mask[1] + out_u_target_subj_bg * (1 - spatial_mask[0]) + out_u_target_obj_bg * (1 - spatial_mask[1])
        #     # out_c_target = out_c_target_subj_fg * spatial_mask[0] + out_c_target_obj_fg * spatial_mask[1] + out_c_target_subj_bg * (1 - spatial_mask[0]) + out_c_target_obj_bg * (1 - spatial_mask[1])
        #
        #     out_u_target_subj_fg, out_u_target_obj_fg, out_u_target_bg = out_u_target.chunk(3)
        #     out_c_target_subj_fg, out_c_target_obj_fg, out_c_target_bg= out_c_target.chunk(3)
        #
