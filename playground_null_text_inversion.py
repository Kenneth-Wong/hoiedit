...
prompts = ["a real photo of boy feeding horse, in the snow",
           "a real photo of boy <R> horse, in the snow"]
...
editor = MutualSelfAttentionControlMaskAuto(STEP, LAYPER, mask_save_dir=masa_save_dir)
regiter_attention_editor_diffusers(model, editor)

# inference the synthesized image
image_masactrl = model(prompts, latents=start_code, guidance_scale=7.5)[-1:]

editor2 = MutualRelControlMaskAuto(STEP, LAYPER, ref_subj_token_idx=[5], ref_obj_token_idx=[7],
                 cur_subj_token_idx=[5], cur_obj_token_idx=[7], mask_save_dir=mare_save_dir, thres=0.1)
